...
from sgpt.utils import get_shell_name
...
def initial(prompt: str, shell: bool, code: bool) -> str:
    ...
    pwd = os.getcwd()
    if shell:
        return SHELL_PROMPT.format(shell=shell_name, os=os_name, prompt=prompt, pwd=pwd)
    if code:
        return CODE_PROMPT.format(prompt=prompt)
    return DEFAULT_PROMPT.format(shell=shell_name, os=os_name, prompt=prompt, pwd=pwd)
