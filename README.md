...
```
### Shell commands
Have you ever found yourself forgetting common shell commands, such as `chmod`, and needing to look up the syntax online? you can quickly find and execute the commands you need right in the terminal.
```shell
sgpt "make all files in current directory read only"
# -> chmod -R a-w .
```
Since we are receiving valid shell command, we can execute it using `eval $(sgpt "make all files in current directory read only")` but this is not very convenient, instead we can use `--execute` (or shortcut `-e`) parameter:
```shell
sgpt --execute "make all files in current directory read only"
# -> chmod -R a-w .
...
```shell
sgpt -e "start nginx using docker, forward 443 and 80 port, mount current folder with index.html"
...
```shell
sgpt -e "slow down video twice using ffmpeg, input video name \"input.mp4\" output video name \"output.mp4\""
...
ls
# -> 1.mp4 2.mp4 3.mp4
sgpt -e "using ffmpeg combine multiple videos into one without audio. Video file names: $(ls -m)"
# -> ffmpeg -i 1.mp4 -i 2.mp4 -i 3.mp4 -filter_complex "[0:v:0][1:v:0][2:v:0]concat=n=3:v=1:a=0" output.mp4
# -> Execute shell command? [y/N]: y
# ...
```
...
╰────────────────────────────────────────────────────────────────────────────────────────────────────────╯
╭─ Options ──────────────────────────────────────────────────────────────────────────────────────────────╮
│ --temperature      FLOAT RANGE [0.0<=x<=1.0]  Randomness of generated output. [default: 0.0]           │
